...
#'
#' The function uses an Empirical Cumulative Distribution (ECD) function to determine the area under the curve of the metric values of the simulated trees, to get to a p-value for the position of the metrics of the empirical tree on that distribution. The argument `metricset` allows to chose which group of metrics to calculate p-values for: `"classic"` - a couple of more 'conventional' measures of tree shape, being Colless index, Sackin index, number of cherries, number of pitchforks, average ladder size, and gamma statistic; `"nodibranch" - includes minimum, maximum, and median for both node ages and branch lengths respectively. If Laplacian spectra were calculated, `"spectR"`- returns the standard (i.e. unnormalised) spectral densities, and `"spectRnorm"` - the normalised spectral densities. Option `"All"` returns all sets, and `"AllQuick"` returns all except the metrics based on the Laplacian spectra. For more information on the spectral densities, i.e. the Eigenvalues of the tree's modified graph Laplacian, see R package RPANDA and associated papers.
#'
...
#' @param metricset String specifying which tree metrics to use; default is "AllQuick", other options are "ALL", "classic", "nodibranch", "spectR", and "spectrRnorm"; for more information on the options see Details.
...
PvalMetrics <- function(empMetrics, simMetrics, empirical_start=TRUE, methodnr, metricset="AllQuick") {
