...
The way BoskR is trying to judge adequacy is by tree shape. The logic behind this is, that if a model cannot produce trees of a shape similar to an empirical tree, it is also not suited to analyse that tree. The package makes use of a number of 'classical' metrics (_e.g._, Colless, node ages). These metrics can also be complemented with those measured from the spectral density of a tree's modified graph Laplacian, as obtained through the package RPANDA^[Details on this can be found in the corresponding publication (Eric Lewitus, Helene Morlon; Characterizing and Comparing Phylogenies from their Laplacian Spectrum, Systematic Biology, Volume 65, Issue 3, 1 May 2016, Pages 495–507, [https://doi.org/10.1093/sysbio/syv116](https://doi.org/10.1093/sysbio/syv116)) and package documentations ([Package RPANDA on CRAN](https://CRAN.R-project.org/package=RPANDA)).], to gain more information on the trees, however, this option has been turned off by default for now, as it can be quite taxing on runtime and memory use.
...
```
Now we can also already get this tree's shape metrics. We set `empirical_start` to `TRUE`, as we used our empirical whale tree as the starting point of the analysis, and leave the remaining options on their default values^[The option in question is `quick_run` which is set to `TRUE`. This ensure that we skip the calculation of the Laplacian spectra, as they can be a bit time-intensive. They can also use quite a bit of memory, thus `return_spectra` is by default set to `FALSE`, which means the actual Laplacian spectra aren't saved, but only the metrics derived from it. Unless you want to dig deeper for some reason, you probably don't need to bother with them anyways.].
```{r, results="hide", warning=FALSE, message=FALSE}
emptestmetrics <- GetTreeMetrics(emptesttrees, empirical_start=TRUE, return_spectra=FALSE, quick_run=TRUE) # get tree shape metrics from empirical trees
```
...
By default, this used the `"classic"` set of metrics (Colless index, Sackin index, number of cherries, number of pitchforks, average ladder size, and the gamma statistic), but we can also look at the set containing number of tips, node ages, and branch lengths, by specifying it accordingly using the argument `metricset`:
```{r, fig.width=7, fig.height=2}
plotPvalMetricsPDF(emptestmetrics, simtestmetrics, set=1, metricset="nodibranch")
```
